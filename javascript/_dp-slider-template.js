...
(function () {
   ...
   * 設定値（パターンごとに変更する）
   * -------------------------------------------------------- */
  const SLIDER_TRIGGER = '.js-works-slider';   // ブロックコードに付与するクラス
  const OUTER_CLASS    = 'dp-works-slider-c-outer'; // outer wrapper クラス
  const LOOP_MODE      = true;                 // ループするか否か
  const MIN_SLIDES     = 2;                    // 最低スライド枚数（dp-works-slider-b は 4）
